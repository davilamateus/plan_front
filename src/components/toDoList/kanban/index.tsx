...
const Kanban = () => {
	...
	const [columns, setColumns] = useState<Record<string, Column>>({
		toDo: { id: 1, name: "A fazer", items: [] },
		inProgress: { id: 2, name: "Em progresso", items: [] },
		done: { id: 3, name: "Finalizado", items: [] },
	});
	...
	useEffect(() => {
		if (toDoList?.state) {
			...
			const toDoItems = toDoList.state
				.filter((item) => item.status === 1)
				.sort((a, b) => a.position - b.position);
			const inProgressItems = toDoList.state
				.filter((item) => item.status === 2)
				.sort((a, b) => a.position - b.position);
			const doneItems = toDoList.state
	...
	const onDragEnd = async (result: DropResult) => {
		...
		if (source.droppableId !== destination.droppableId) {
			...
			const updatedSourceItems = sourceItems.map((item, index) =>
				item.position !== index
					? { ...item, position: index, status: sourceColumn.id }
					: item
			);
			const updatedDestItems = destItems.map((item, index) =>
				item.position !== index
					? { ...item, position: index, status: destColumn.id }
					: item
			);
			...
			<div className="todolist-table">
				<DragDropContext onDragEnd={onDragEnd}>
					{Object.entries(columns).map(([columnId, column]) => (
						<div className="todolist-columm" key={columnId}>
...
const SkeletonCards = () => (
	<>
		{[...Array(3)].map((_, index) => (
			<div className="todolist-card box" key={index}>
