            ...
            <label>
                ...
                {tasks && tasks.length > 0 ? (
                    tasks.map((task: IToDoListTasks) => (
                        <div
                            key={task.id}
                            className={`task-card ${task.status == true ? "task-done" : ""}`}>
                            <div
                                className="task-left"
                                onClick={() => toDoListContext?.editTask(todolistId, task.id, !task.status)}>
                            ...
                            <div className="task-right">
                                <ButtonDeleteSmall action={() => toDoListContext?.deleteTask(todolistId, task.id)} />
                            </div>
                        </div>
                    ))
                ) : (
