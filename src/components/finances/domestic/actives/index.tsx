...
const Actives = ({ date }: type) => {
	...
	const [finances, setFinances] = useState<
		IFinancesEntrace[] | IFinancesExpense[]
	>([]);
	...
	useEffect(() => {
		if (context?.state.trip.expenses) {
			const newArray = context?.state.entraces
				.concat(
					context?.state.domestic.expenses.concat(context.state.trip.expenses)
				)
	...
	const isExpense = (
		item: IFinancesEntrace | IFinancesExpense
	): item is IFinancesExpense => {
		return (item as IFinancesExpense).type !== undefined;
	};

	return (
		<div className="finances-actives">
			<TitleOfSession title="Atividades" />
			<div className="finances-actives-table box">
				{finances.length > 0 ? (
						...
						<tbody>
							{finances.map((item, index) => (
								...
								>
									...
									<td style={{ color: item.color }} className="value">
					...
					<div className="no-activities">Nenhuma atividade.</div>
				)}

				{opened && (
					<BoxFullpage
						title={finances[index].title}
						content={
							<>
