...
const Weather = () => {
	...
	useEffect(() => {
		if (trip?.state.tripCity) {
			UseGetWeather(trip?.state.tripCity, trip?.state.tripCountrySlug).then(
				(data) => {
					setWeather(data);
				}
			);
			...
			<div className="weather-left">
				<div className="weather-date">
					<span>
						{
							days[
								new Date(dateNow.getTime() + weather.timezone * 1000).getDay()
							]
						}
						,
					</span>
					<span>
						{new Date(dateNow.getTime() + weather.timezone * 1000).getDate()}
					</span>
					<span>
						{
							getTimestampInfomartions(
								new Date(dateNow.getTime() + weather.timezone * 1000),
								0
							).nameOfMonthShort
						}
						,
					...
					<span>
						...
						{new Date(
							dateNow.getTime() + weather.timezone * 1000
						).getMinutes() < 10
							? "0"
							: ""}
				...
				<div className="weather-degre">
					<h3 className="weather-degre-number">
