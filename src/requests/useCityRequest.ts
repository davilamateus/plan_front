...
export const useGetNoticies = () => {
	return async (country: string, category?: string, page?: string) => {
		return await Api.get(
			`/cities/noticies?country=${country}${
...
export const useGetAdvices = () => {
	return async (lat: string, lon: string, category: string) => {
		return await Api.get(
			`/cities/advices?lat=${lat}&lon=${lon}&category=${category}`
		)
			.then((data) => {
				return data.data;
			})
...
export const useGetExchange = () => {
	return async (tripCurrency: string, currentCurrency: string) => {
		return await Api.get(
			`cities/exchange?tripCurrency=${tripCurrency}&currentCurrency=${currentCurrency}`
		)
			.then((data) => {
				return data;
			})
